...
class SportswearService {
 ...
 Future<Product> updateBrand(Product existingBrand) async {
   ...
   if (response.statusCode == 200) {
     ...
     final errorBody = jsonDecode(response.body);
     throw Exception('Validation Failed: ${errorBody['errors'] ?? 'Invalid data'}');
   } else {
 ...
 void _showTagFilterDialog() {
   showDialog(
     ...
     builder: (BuildContext context) {
       return AlertDialog(
         title: const Text('Filter by Category', style: TextStyle(color: darkBlueColor)),
           ...
           child: ListView.builder(
             ...
             itemBuilder: (context, index) {
               ...
               return RadioListTile<String>(
                 title: Text(tag, style: const TextStyle(color: darkBlueColor)),
                 value: tag,
               ...
               child: FutureBuilder<List<Product>>(
                 ...
                 builder: (context, snapshot) {
                   if (snapshot.connectionState == ConnectionState.waiting) {
                     return const Center(child: CircularProgressIndicator(color: darkBlueColor));
                   } else if (snapshot.hasError) {
                     return Center(
                       child: Padding(
                         padding: const EdgeInsets.all(20.0),
                         child: Text('Failed to load data: ${snapshot.error}', style: const TextStyle(color: Colors.red)),
                       ),
                     );
                   } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                     return Center(
                       child: Text(
                         _searchQuery.isEmpty && _selectedTagFilter == null ? 'No brands found.' : 'No results found.',
                   ...
                   children: [
                     ...
                     Row(
                       ...
                       children: [
                         ...
                         Icon(Icons.bookmark_border, color: darkBlueColor.withOpacity(0.7), size: 20),
                       ],
                     ),
                     ...
                     Padding(
                       ...
                       child: Text(
                         product.description,
                         style: TextStyle(fontSize: 12, color: darkBlueColor.withOpacity(0.7)),
                         maxLines: 2,
                         overflow: TextOverflow.ellipsis,
                       ),
                     ),
                       ...
                       child: Row(
                         children: [
                           ...
                           Text(
                             ...
                             style: const TextStyle(fontWeight: FontWeight.bold, color: darkBlueColor, fontSize: 12),
                           ),
                           ...
                           Container(
                             ...
                             child: Text(
                               product.tag,
                               style: const TextStyle(fontSize: 11, color: whiteColor, fontWeight: FontWeight.w600),
                             ),
                           ),
                         ],
                       ),
           ...
           child: Align(
             ...
             child: InkWell(
               ...
               child: const Row(
                 ...
                 children: [
                   Text('Details', style: TextStyle(color: orangeColor, fontWeight: FontWeight.bold, fontSize: 14)),
                   SizedBox(width: 2),
             ...
             child: Column(
               ...
               children: [
                 Row(
                   mainAxisAlignment: MainAxisAlignment.spaceBetween,
                   children: [
                     const Text('Timeline Reviews', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: darkBlueColor)),
                     Icon(_showReviews ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, color: darkBlueColor, size: 20),
                   ],
                 ),
                 if (_showReviews)
                   if (reviews.isEmpty)
                     Padding(
                       ...
                       child: Text('Belum ada ulasan.', style: TextStyle(color: darkBlueColor.withOpacity(0.6), fontSize: 12)),
                     )
                   else
   ...
   return Container(
     ...
     child: Column(
       ...
       children: [
         Row(
           children: [
             ...
             Column(
               crossAxisAlignment: CrossAxisAlignment.start,
               children: [
                 Text(review.username, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: darkBlueColor)),
                 Row(
                   children: [
                     Icon(Icons.location_on, size: 10, color: darkBlueColor.withOpacity(0.6)),
                     Text(review.location, style: TextStyle(color: darkBlueColor.withOpacity(0.6), fontSize: 10)),
                     const SizedBox(width: 6),
                     const Icon(Icons.star, size: 10, color: orangeColor),
                     Text(review.ratingValue.toStringAsFixed(1), style: const TextStyle(color: darkBlueColor, fontSize: 10, fontWeight: FontWeight.bold)),
                   ],
                 ),
               ],
             ),
         ...
         const SizedBox(height: 8),
         Text(review.reviewText, style: const TextStyle(fontSize: 12, color: darkBlueColor)),
       ],
     ),
   );
 }
